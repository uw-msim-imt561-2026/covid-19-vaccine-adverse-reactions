...
def header_metrics(df: pd.DataFrame) -> None:
    ...
    st.markdown("The percent change for each of the above metrics is calculated between the **last month** " + "(" + str(get_last_month(df)) + ")" + " and **immediate previous month** " + "(" + str(get_prev_month(df)) + ")" + " of the selected date range.")
