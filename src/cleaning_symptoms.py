...
def reshape_symptoms(df) -> pd.DataFrame:
    """Reshapes VAERSSYMPTOMS.csv to be more manageable for analysis."""
    # convert SYMPTOM / SYMPTOMVERSION pairs to dictionary item in series
    VAERS_IDs_dict = df[['VAERS_ID']].apply(lambda row: row.to_dict(), axis=1)
    symptom1_dict = df[['SYMPTOM1', 'SYMPTOMVERSION1']].apply(lambda row: row.to_dict(), axis=1)
    symptom2_dict = df[['SYMPTOM2', 'SYMPTOMVERSION2']].apply(lambda row: row.to_dict(), axis=1)
    symptom3_dict = df[['SYMPTOM3', 'SYMPTOMVERSION3']].apply(lambda row: row.to_dict(), axis=1)
    symptom4_dict = df[['SYMPTOM4', 'SYMPTOMVERSION4']].apply(lambda row: row.to_dict(), axis=1)
    symptom5_dict = df[['SYMPTOM5', 'SYMPTOMVERSION5']].apply(lambda row: row.to_dict(), axis=1)

    # create subset dataset of VAERS_IDs
    df_VAERS_IDs = df['VAERS_ID'].to_frame()
    ...
    df_symptoms_reshaped = df_symptoms_reshaped.set_axis(['VAERS_ID', 'symptom', 'symptom_version', 'symptom_dict'],
                                                         axis=1)
    return df_symptoms_reshaped

def drop_null_symptoms(df: pd.DataFrame) -> pd.DataFrame:
    df = df.dropna()
    return df

def handle_whitespace_symptoms(df: pd.DataFrame) -> pd.DataFrame:
